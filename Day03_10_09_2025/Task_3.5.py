...
def countFrequency() :
    chaine = input().replace(' ', '').lower()
    scoringLanguage = {'english': 0, 'french': 0, 'spanish': 0, 'portuguese' : 0, 'italian': 0, 'german': 0, 'dutch' : 0, 'swedish': 0, 'polish': 0, 'turkish': 0, 'esperanto': 0}
    compare = -1
    langChoose = ''
    characChoose = ''
    language = ['english', 'french', 'spanish', 'portuguese', 'italian', 'german', 'dutch', 'swedish', 'polish', 'turkish', 'esperanto']
    
    for charac in chaine :
        for lang in language :
            if charac in LANGUAGE_PROFILES[lang] and compare > abs(chaine.count(charac) - LANGUAGE_PROFILES[lang][charac]) or compare == -1 :
                compare = abs(chaine.count(charac) - LANGUAGE_PROFILES[lang][charac])
                langChoose = lang
                characChoose = charac
        scoringLanguage[langChoose] += LANGUAGE_PROFILES[langChoose][characChoose]
    print(scoringLanguage)
